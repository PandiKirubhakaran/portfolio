...
const ProgressBar = ({ skills }) => {
  ...
  useEffect(() => {
    skills.forEach((skill) => {
      setTimeout(() => {
        document.getElementById(`${skill.name}pourcent`).innerHTML = skill.percentage;
        document.getElementById(`progress${skill.name}`).style.width = skill.percentage;
      ...
      {skills.map((skill) => (
        <li key={skill.name}>
          <div className={classes.progressbarTitle}>
            <h3>{skill.name}</h3>
            <span className={classes.percent} id={`${skill.name}pourcent`}></span>
          </div>
